  ...
  <div class="view">
    <header class="header">
      ...
      <div class="header__items">
        ...
        <await name="listExportCampaing" :spinnerScale="0.8">
          <button @click="downloadXlsx" class="btn btn--solid">
            {{ $t('app.downloadXlsx') }}
          </button>
        </await>

        <router-link to="/campaing/new" class="btn btn--contrast bg-secondary">
          {{ $t('app.add') }}
        </router-link>
      </div>
...
import {ListCampaingSchema} from '@/schema/resource/Campaing/ListCampaingSchema'
import {ExportCampaingSchema} from '@/schema/resource/Campaing/ExportCampaingSchema'

@Component({
  components: {FilterToggle, FilterCampaing},
})
export default class ListCampaingView extends Mixins(MixinRouteMatch) {
  schema = new ListCampaingSchema()
  ...
  goToPersistView(item: Campaing) {
    this.$nav.pushByName('editCampaing', item.$id)
  }

  async downloadXlsx() {
    const {ascending, orderBy, page, limit, ...params} = this.collection.params

    const coll = new CampaingCollection().clearFilters().addFilter(params)

    await coll.listExportCampaing()
    this.$xlsx.downloadFromSchema(coll.items, new ExportCampaingSchema())
  }
}
</script>
