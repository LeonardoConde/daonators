  ...
  <div class="view">
    <header class="header">
      ...
      <div class="header__items">
        ...
        <await name="listVoting" :spinnerScale="0.8" class="w-12" />
        ...
        <await name="listExportVoting" :spinnerScale="0.8">
          <button @click="downloadXlsx" class="btn btn--solid">
            {{ $t('app.downloadXlsx') }}
          </button>
        </await>

        <router-link to="/voting/new" class="btn btn--contrast bg-secondary">
          {{ $t('app.add') }}
        </router-link>
      </div>
      ...
      <transition-expand>
        <div v-show="filterOpen" class="z-10">
          <filter-voting :collection="collection" />
        </div>
      </transition-expand>
    </header>
...
import FilterVoting from '@/components/filters/FilterVoting.vue'
import {Voting} from '@/model/resource/Voting'
import {VotingCollection} from '@/model/collection/VotingCollection'
import {ListVotingSchema} from '@/schema/resource/Voting/ListVotingSchema'
import {ExportVotingSchema} from '@/schema/resource/Voting/ExportVotingSchema'

@Component({
  components: {FilterToggle, FilterVoting},
})
export default class ListVotingView extends Mixins(MixinRouteMatch) {
  schema = new ListVotingSchema()
  ...
  goToPersistView(item: Voting) {
    this.$nav.pushByName('editVoting', item.$id)
  }

  async downloadXlsx() {
    const {ascending, orderBy, page, limit, ...params} = this.collection.params

    const coll = new VotingCollection().clearFilters().addFilter(params)

    await coll.listExportVoting()
    this.$xlsx.downloadFromSchema(coll.items, new ExportVotingSchema())
  }
}
</script>
