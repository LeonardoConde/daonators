  ...
  <div class="view">
    <header class="header">
      ...
      <div class="header__items">
        ...
        <await name="listExportCampaingOrganization" :spinnerScale="0.8">
      ...
      <transition-expand>
        <div v-show="filterOpen" class="z-10">
          <filter-campaing-organization :collection="collection" />
        </div>
      </transition-expand>
    </header>
...
import FilterCampaingOrganization from '@/components/filters/FilterCampaingOrganization.vue'
...
import {ListCampaingOrganizationSchema} from '@/schema/resource/CampaingOrganization/ListCampaingOrganizationSchema'
import {ExportCampaingOrganizationSchema} from '@/schema/resource/CampaingOrganization/ExportCampaingOrganizationSchema'

@Component({
  components: {FilterToggle, FilterCampaingOrganization},
})
export default class ListCampaingOrganizationView extends Mixins(
  MixinRouteMatch
) {
  schema = new ListCampaingOrganizationSchema()
  collection = new CampaingOrganizationCollection()
  filterOpen = false
  ...
  goToPersistView(item: CampaingOrganization) {
    this.$nav.pushByName(
      'editCampaingOrganization',
      item.idCampaingFk,
      item.idOrganizationFk
    )
  }
  ...
  async downloadXlsx() {
    ...
    const coll = new CampaingOrganizationCollection()
      .clearFilters()
      .addFilter(params)

    await coll.listExportCampaingOrganization()
    this.$xlsx.downloadFromSchema(
      coll.items,
      new ExportCampaingOrganizationSchema()
    )
  }
}
</script>
