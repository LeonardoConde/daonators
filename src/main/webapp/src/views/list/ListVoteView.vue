  ...
  <div class="view">
    <header class="header">
      ...
      <div class="header__items">
        ...
        <await name="listVote" :spinnerScale="0.8" class="w-12" />
        ...
        <await name="listExportVote" :spinnerScale="0.8">
          <button @click="downloadXlsx" class="btn btn--solid">
            {{ $t('app.downloadXlsx') }}
          </button>
        </await>

        <router-link to="/vote/new" class="btn btn--contrast bg-secondary">
          {{ $t('app.add') }}
        </router-link>
      </div>
      ...
      <transition-expand>
        <div v-show="filterOpen" class="z-10">
          <filter-vote :collection="collection" />
        </div>
      </transition-expand>
    </header>
...
import FilterVote from '@/components/filters/FilterVote.vue'
import {Vote} from '@/model/resource/Vote'
import {VoteCollection} from '@/model/collection/VoteCollection'
import {ListVoteSchema} from '@/schema/resource/Vote/ListVoteSchema'
import {ExportVoteSchema} from '@/schema/resource/Vote/ExportVoteSchema'

@Component({
  components: {FilterToggle, FilterVote},
})
export default class ListVoteView extends Mixins(MixinRouteMatch) {
  schema = new ListVoteSchema()
  ...
  async downloadXlsx() {
    ...
    const coll = new VoteCollection().clearFilters().addFilter(params)

    await coll.listExportVote()
    this.$xlsx.downloadFromSchema(coll.items, new ExportVoteSchema())
  }
}
</script>
