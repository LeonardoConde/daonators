  ...
  <div class="view">
    <header class="header">
      ...
      <div class="header__items">
        ...
        <await name="listAuthAdm" :spinnerScale="0.8" class="w-12" />
        ...
        <await name="listExportAuthAdm" :spinnerScale="0.8">
          <button @click="downloadXlsx" class="btn btn--solid">
            {{ $t('app.downloadXlsx') }}
          </button>
        </await>

        <router-link to="/auth-adm/new" class="btn btn--contrast bg-secondary">
          {{ $t('app.add') }}
        </router-link>
      </div>
      ...
      <transition-expand>
        <div v-show="filterOpen" class="z-10">
          <filter-auth-adm :collection="collection" />
        </div>
      </transition-expand>
    </header>
...
import FilterAuthAdm from '@/components/filters/FilterAuthAdm.vue'
import {AuthAdm} from '@/model/resource/AuthAdm'
import {AuthAdmCollection} from '@/model/collection/AuthAdmCollection'
import {ListAuthAdmSchema} from '@/schema/resource/AuthAdm/ListAuthAdmSchema'
import {ExportAuthAdmSchema} from '@/schema/resource/AuthAdm/ExportAuthAdmSchema'

@Component({
  components: {FilterToggle, FilterAuthAdm},
})
export default class ListAuthAdmView extends Mixins(MixinRouteMatch) {
  schema = new ListAuthAdmSchema()
  collection = new AuthAdmCollection()
  filterOpen = false
  ...
  goToPersistView(item: AuthAdm) {
    this.$nav.pushByName('editAuthAdm', item.$id)
  }

  async downloadXlsx() {
    const {ascending, orderBy, page, limit, ...params} = this.collection.params

    const coll = new AuthAdmCollection().clearFilters().addFilter(params)

    await coll.listExportAuthAdm()
    this.$xlsx.downloadFromSchema(coll.items, new ExportAuthAdmSchema())
  }
}
</script>
